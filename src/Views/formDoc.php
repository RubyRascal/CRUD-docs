...
<form class="formCreate" method="POST" action="">
    ...
    <input class="input" type="date" name="date-start" value="<?php if (isset($_POST["submitCreate"])) {echo $_POST["dateStart"];}else{echo $DocsData["date-start"];}?>"/><p class="error"><?php echo $errors["dateStartErr"] ?></p>
    <h3 class="form-text">Contract finish date</h3>
    <input class="input" type="date" name="date-finish" value="<?php if (isset($_POST["submitCreate"])) {echo $_POST["dateFinish"];}else{echo $DocsData["date-finish"];}?>"/><p class="error"><?php echo $errors["dateFinishErr"] ?></p>
    ...
    <input class="input" type="ur-addres" placeholder="Legal address" name="ur-addres" value="<?php if (isset($_POST["submitCreate"])) {echo $_POST["urAddress"];}else{echo $DocsData["urAddress"];}?>"/><p class="error"><?php echo $errors["urAddresErr"] ?></p>
    <!-- <p>Юр. адрес совпадает с физ. адресом?</p><input type="checkbox"> -->
    <input class="input" type="fiz-addres" placeholder="Physical address" name="fiz-addres" value="<?php if (isset($_POST["submitCreate"])) {echo $_POST["fizAddress"];}else{echo $DocsData["fizAddress"];}?>"/><p class="error"><?php echo $errors["fizAddresErr"] ?></p>
    ...
    <input class="input" type="submit" <?php if (isset($_GET["id"])) {?> name="submitUpdate"<?php }else{ ?> name="submitCreate"<?php } ?> <?php if (isset($_GET["id"])) {?> value="Edit"<?php }else{ ?> value="Create"<?php } ?>/>
</form>
</body>
